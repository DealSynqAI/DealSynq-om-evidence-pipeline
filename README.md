...
It attempts full-page Qwen vision on every selected page, retaining a failure receipt if a call cannot produce a valid proposal. It runs PP-StructureV3 on every likely table page, including pages where the native PDF parser already found tables, and on pages with low-confidence prose that needs a second OCR reading. Native table cells remain authoritative on disagreement; Paddle-detected image tables are retained for review. Every run writes a new immutable directory and performs independent schema/integrity validation.
