...
The ownership stage separates page-wide background bands from overlapping content images, rejects sparse page-frame grids that lack repeated populated rows, and scores competing OCR region owners using table cell boxes where available. Ambiguous competing region assignments remain visible in `reconciliation/page-NNN.json` under `region_ownership_decisions`. PaddleOCR's cell boxes are retained alongside its HTML grid; an image-table value is promoted to OCR-grounded only when its printed row label and value independently match the corresponding cell geometry. Remaining candidate cells continue to require review.
